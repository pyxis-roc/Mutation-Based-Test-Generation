...
def run_tests(wp, insn, muthelper, experiment, round2 = False):
    tt = InsnTest(wp, insn)
    tt.load_tests()
...
if __name__ == "__main__":
    ...
    p.add_argument("--round2", help="Second round, so run only on survivors using tests generated by eqvcheck", action='store_true')
    ...
    if len(insns):
        ...
        for i in insns:
            insn = Insn(i)
            survivors = run_tests(wp, insn, muthelper, args.experiment, args.round2)
            print(f"{len(survivors)} survivors for {i}")

            if args.round2:
                ofile = wp.workdir / insn.working_dir / f"mutation-testing.round2.{args.experiment}.json"
            else:
