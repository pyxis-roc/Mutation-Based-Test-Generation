...
/* these remove the generics, since MUSIC seems unable to handle
 * _Generic and silently produces fewer mutations. Note these
 * substitutions are semantically different from the originals. */
...
#define extract_24(X) extract_24_signed(X)

#define MAXVALUE(x) UINT64_MAX

#define ABSOLUTE(x) labs(x)

#define INTWIDTH(x) 64
...
#ifndef PYCPARSER
#define RCP(X) RCP_DOUBLE(X)

#define RSQRT(X) RSQRT_DOUBLE(X)
